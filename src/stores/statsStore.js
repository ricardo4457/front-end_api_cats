...
export const useStatsStore = defineStore('StatsStore', () => {
  ...
  const fetchTopQueries = async () => {
    try {
      loading.value = true
      const response = await axios.get('http://192.168.1.75:3000/api/v1/stats/top-queries')
  ...
  const fetchTopCategories = async () => {
    try {
      loading.value = true
      const response = await axios.get('http://192.168.1.75:3000/api/v1/stats/top-categories')
  ...
  const fetchCatsByCategory = async (tag) => {
    try {
      ...
      const response = await axios.get(
        `http://192.168.1.75:3000/api/v1/stats/category/${encodeURIComponent(tag)}`,
      )
