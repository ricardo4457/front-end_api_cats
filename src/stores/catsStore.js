...
export const useCatsStore = defineStore('catsStore', () => {
  ...
  const fetchAllTags = async () => {
    try {
      const response = await axios.get('http://192.168.1.75:3000/api/v1/tags')
  ...
  const searchTagsBySubstr = async (search) => {
    ...
    try {
      const response = await axios.get(`http://192.168.1.75:3000/api/v1/cats/match?string=${search}`)
  ...
  const searchCatsByTag = async (tag, page = 1) => {
    ...
    try {
      const response = await axios.get(`http://192.168.1.75:3000/api/v1/cats/filter`, {
        params: {
          tag: tag,
